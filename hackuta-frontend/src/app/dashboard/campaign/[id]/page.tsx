...
export default function CampaignDetailPage({ params }: CampaignDetailPageProps) {
  ...
  const { getCampaignById, getCampaignAds } = useCampaigns();
    ...
    return (
      <div className="min-h-screen bg-background text-foreground">
        ...
        <main className="mx-auto flex w-full max-w-4xl flex-col items-center gap-4 px-6 py-16 text-center">
          <h1 className="text-2xl font-heading font-semibold text-navy">Campaign not found</h1>
          <p className="text-sm text-slate-500">This campaign may have been removed or is unavailable.</p>
          <button
            type="button"
            onClick={() => router.push("/dashboard")}
    ...
    <div className="min-h-screen bg-background text-foreground">
      <header className="bg-gradient-to-r from-slate-50 via-white to-blue-50 border-b border-slate-200">
        <div className="mx-auto flex w-full max-w-6xl items-center justify-between px-6 py-6 sm:px-8 lg:px-12">
          ...
          <Link href="/dashboard" className="text-sm font-medium text-accent transition hover:text-accent-hover">
            ← Back to Dashboard
          </Link>
        </div>
      </header>
      ...
      <main className="mx-auto flex w-full max-w-6xl flex-col gap-12 px-6 py-12 sm:px-8 lg:px-12">
        <section className="space-y-2 text-center">
          <h1 className="text-3xl font-heading font-semibold text-slate-800">{campaign.name}</h1>
        ...
        <section className="rounded-2xl border border-slate-200 bg-white/95 p-8 shadow-sm">
          ...
          <p className="mt-3 text-center text-xs text-slate-400">
            {hasAds ? "Campaign averages (auto-updated)." : "Awaiting creatives to compute insights."}
        ...
        <section className="grid gap-8 lg:grid-cols-[2fr_1fr]">
          <div className="space-y-8">
            <AdUploader campaignId={campaign.id} />
            {hasAds ? null : (
              <p className="text-center text-sm text-slate-500">Upload your first creative to generate insights.</p>
            )}
            {hasAds ? (
              <div className="space-y-6">
                <div className="flex items-center gap-3 text-sm text-slate-500">
                  <span>Filters:</span>
                  <button type="button" className="rounded-full border border-slate-200 px-3 py-1 text-xs text-slate-500" disabled>
                    All
                  </button>
                  <button type="button" className="rounded-full border border-slate-200 px-3 py-1 text-xs text-slate-400" disabled>
                    Needs Analysis
                  </button>
                  <button type="button" className="rounded-full border border-slate-200 px-3 py-1 text-xs text-slate-400" disabled>
                    High Potential
          ...
          <div className="flex flex-col gap-6">
            {campaign.description ? (
              <section className="rounded-2xl border border-slate-200 bg-white/95 p-6 shadow-sm">
                <h2 className="text-lg font-semibold text-slate-700">Campaign Description</h2>
            ...
            {campaign.contextAnswers ? (
              <section className="rounded-2xl border border-slate-200 bg-white/95 p-6 shadow-sm">
                <h2 className="text-lg font-semibold text-slate-700">Context Notes</h2>
                <div className="mt-4 space-y-3">
                  {campaign.contextAnswers.emotion ? (
                    <div>
                      <p className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">Emotion Target</p>
                      <p className="mt-2 text-sm text-slate-600">{campaign.contextAnswers.emotion}</p>
                    </div>
                  ) : null}
                  {campaign.contextAnswers.success ? (
                    <div>
                      <p className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">Success Signal</p>
                      <p className="mt-2 text-sm text-slate-600">{campaign.contextAnswers.success}</p>
                    </div>
                  ) : null}
                  {campaign.contextAnswers.inspiration ? (
                    <div>
                      <p className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">Inspiration</p>
                      <p className="mt-2 text-sm text-slate-600">{campaign.contextAnswers.inspiration}</p>
                    </div>
                  ) : null}
                </div>
              </section>
