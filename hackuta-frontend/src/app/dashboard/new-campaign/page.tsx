...
export default function NewCampaignPage() {
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    // Dummy: local push; TODO [backend]: POST to /api/campaigns/create with full description + context.
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <main className="mx-auto flex w-full max-w-3xl flex-col gap-8 px-6 py-16 sm:px-8">
        <div className="space-y-2">
          <h1 className="text-3xl font-heading font-semibold text-slate-800">Create a New Campaign</h1>
          <p className="text-sm text-slate-500">
            Define your campaign’s purpose and give our models the right context to critique it effectively.
          </p>
        </div>
        ...
        <form className="flex flex-col gap-8" onSubmit={handleSubmit}>
          <section className="space-y-2">
            ...
            {name.length > 0 && nameError && (
              <p className="text-xs text-red-500">Name must be between 3 and 50 characters.</p>
            )}
          </section>
          ...
          <section className="space-y-2">
            ...
            <div className="flex items-center justify-between text-xs text-slate-400">
              ...
              {descriptionCount > 0 && descriptionError && <span className="text-red-500">Add at least 50 characters.</span>}
            </div>
          ...
          <section className="space-y-4">
            <p className="text-sm font-semibold text-navy">Context Questions (Optional)</p>
            <p className="text-xs text-slate-500">
              {/* Dummy: these responses will help seed AI critique prompts later. */}
              Provide additional context to help our models tailor their critique.
            </p>

            <div className="space-y-4">
              <div className="space-y-2">
                <label htmlFor="emotions" className="text-sm font-medium text-slate-700">
                ...
                <textarea
                  ...
                  onChange={(event) =>
                    setContext((prev) => ({ ...prev, emotion: event.target.value }))
                  }
              ...
              <div className="space-y-2">
                ...
                <textarea
                  ...
                  onChange={(event) =>
                    setContext((prev) => ({ ...prev, success: event.target.value }))
                  }
              ...
              <div className="space-y-2">
                <label htmlFor="inspirations" className="text-sm font-medium text-slate-700">
                  Are there any design or messaging inspirations to keep in mind?
                </label>
                <textarea
                  id="inspirations"
                  rows={2}
                  ...
                  onChange={(event) =>
                    setContext((prev) => ({ ...prev, inspiration: event.target.value }))
                  }
